...
public class StopWordRemoval {
	...
	static String set = "abcdefghijklmnopqrstuvwxyz0123456789";
	
	public static List<String> removeStopWords(String[] wordsArray){
		...
		for(int i=0;i<wordsArray.length;i++){
			...
			if(!wordsArray[i].isEmpty()){	//Don't add blanks
				if(wordsArray[i].length()>1){
					for(int j=0;j<wordsArray[i].length();j++){
						if(set.indexOf(wordsArray[i].charAt(j)) == -1){
							wordsArray[i] = wordsArray[i].substring(j+1); //remove all non-letters and non numbers character							
						}
					}					
				}
				
				else if(set.indexOf(wordsArray[i].charAt(0)) == -1){	//change to a stop-word so it gets removed
					wordsArray[i] = "a";
				}
		...
		for (String word : wordsArray) {
			if ( stopWordsList.contains(word)) {
				wordsList.remove(word);	//delete stop-word from list
			}			
		}
		System.out.println("Returned from stopwordremover: "+wordsList.toString());
		return wordsList;		
	}
}
